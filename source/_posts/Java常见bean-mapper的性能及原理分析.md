title: Java常见bean mapper的性能及原理分析
author: alben.wong
abbrlink: f6a7daea
tags:
  - beanmapper
...
categories:
  - java
  - orika
date: 2020-07-07 15:19:00
keywords: orika beanmapper
description: 在分层的代码架构中，层与层之间的对象避免不了要做很多转换、赋值等操作，这些操作重复且繁琐，于是乎催生出很多工具来优雅，高效地完成这个操作，有BeanUtils、BeanCopier、Dozer、Orika等等，本文将讲述上面几个工具的使用、性能对比及原理分析。
---
## 背景
...
![upload successful](/images/Java常见bean mapper的性能及原理分析.png)
...
这个工具可能是大家日常使用最多的，因为是`Spring`自带的，使用也简单：
`BeanUtils.copyProperties(sourceVO, targetVO);`
...
Dozer就提供了这些功能，有支持同名隐式映射，支持基本类型互相转换，支持显示指定映射关系，支持exclude字段，支持递归匹配映射，支持深度匹配，支持Date to String的date-formate，支持自定义转换Converter，支持一次mapping定义多处使用，支持EventListener事件监听等等。不仅如此，Dozer在使用方式上，除了支持API，还支持XML和注解，满足大家的喜好。更多的功能可以参考[这里](https://dozermapper.github.io/gitbook/index.html)
...
Orika可以说是几乎集成了上述几个工具的优点，不仅具有丰富的功能，底层使用`Javassist`生成字节码，运行 效率很高的。



### 使用

Orika基本支持了Dozer支持的功能，这里我也是简单介绍一下Orika的使用，具体更详细的API可以参考[User Guide](http://orika-mapper.github.io/orika-docs/index.html)。
